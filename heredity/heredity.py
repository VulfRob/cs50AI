...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        # Определяем, сколько генов у человека
        ...
        # 1. Вероятность по генам
        if mother is None and father is None:
            # Без родителей: базовая вероятность
            gene_prob = PROBS["gene"][genes]
        else:
            # С родителями: считаем вероятность передачи от каждого
        ...
        # 2. Вероятность проявления признака
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    for person in probabilities:
        # Обновляем вероятность по генам
        if person in two_genes:
        ...
        # Обновляем вероятность по признаку
